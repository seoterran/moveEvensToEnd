    ...
    static Node * moveEvensToEnd(Node * in) {
        ...
        while(ori_node){
            ...
            if(ori_node->val%2==0){ //the current node is even
                if(ori_node!=ori_list) //the first node is not even
                {
                    pre_node->next=ori_node->next;
                ...
                else // the first node is even
                {
                    ori_list=ori_node->next;
                    if(!even_list) {
                        even_list = ori_node;
                        even_node = ori_node;
                    }else if(!even_list->next){
                        even_node->next=ori_node;
                        even_node=ori_node;
                        even_list->next=ori_node;
                    }else{
                        even_node->next=ori_node;
                        even_node=ori_node;
                    }
...
class Unit_Test{
  public:
    static char * test_moveEvensToEnd(){
        ...
        mu_assert((char*)"test_removeFirst_error, result != [1, 5, 3, 1, 3, 6, 4]", compare_list(Node::moveEvensToEnd(in),answer)==0);
        Node::releaseAll(in);
        ...
        mu_assert((char*)"test_removeFirst_error, result != [1, 3, 1, 2, 6, 4, 8]", compare_list(Node::moveEvensToEnd(in),answer)==0);
        Node::releaseAll(in);
        ...
        mu_assert((char*)"test_removeFirst_error, result != [1, 1, 1, 1, 1, 1, 2]", compare_list(Node::moveEvensToEnd(in),answer)==0);
        Node::releaseAll(in);
        ...
        mu_assert((char*)"test_removeFirst_error, result != [1]", compare_list(Node::moveEvensToEnd(in),answer)==0);
        Node::releaseAll(in);
        ...
        mu_assert((char*)"test_removeFirst_error, result != []", compare_list(Node::moveEvensToEnd(in),answer)==0);
        Node::releaseAll(in);
